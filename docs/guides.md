...
| Webhook job shows as **Cancelled** in history | Another job was already running when the webhook delay expired | Wait for the active job to finish; webhooks fired while idle will run normally. To prevent this, increase the webhook delay so imports do not fire during long processing runs. |
