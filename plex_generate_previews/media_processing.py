...
def generate_images(
    ...
    if media_info.video_tracks:
        if (
            media_info.video_tracks[0].hdr_format != "None"
            and media_info.video_tracks[0].hdr_format is not None
        ):
    ...
    if rc != 0 and image_count == 0 and gpu is not None:
        if _detect_codec_error(rc, stderr_lines):
            ...
            logger.warning(
                f"GPU processing failed with codec error (exit code {rc}) for {video_file}; will hand off to CPU worker"
            )
            logger.debug(f"FFmpeg stderr excerpt (last 5 lines): {stderr_excerpt}")
            ...
            raise CodecNotSupportedError(
                f"Codec not supported by GPU for {video_file} (exit code {rc})"
            )
