...
class SettingsManager:
    ...
    def _save_setup_state(self) -> None:
        """Save setup wizard state to file."""
        try:
            self.config_dir.mkdir(parents=True, exist_ok=True)
    ...
    def plex_config_folder(self) -> Optional[str]:
        return self.get('plex_config_folder') or os.environ.get('PLEX_CONFIG_FOLDER', '/plex')
    ...
    def plex_videos_path_mapping(self) -> Optional[str]:
        return self.get('plex_videos_path_mapping') or os.environ.get('PLEX_VIDEOS_PATH_MAPPING')
    ...
    def plex_local_videos_path_mapping(self) -> Optional[str]:
        return self.get('plex_local_videos_path_mapping') or os.environ.get('PLEX_LOCAL_VIDEOS_PATH_MAPPING')
    ...
    def thumbnail_interval(self) -> int:
        return int(self.get('thumbnail_interval') or os.environ.get('THUMBNAIL_INTERVAL', '2'))
    ...
    def thumbnail_quality(self) -> int:
        ...
        return int(self.get('thumbnail_quality') or os.environ.get('THUMBNAIL_QUALITY', '4'))
