...
async function refreshStatus() {
    try {
        ...
        // Show error state in system status
        ...
        if (runningJob) {
            ...
            if (currentJobId) {
                ...
                if (completedJob && completedJob.status !== 'running') {
                    // Job finished - show notification
        ...
        for (const lib of libraries) {
            ...
            html += `
                <div class="library-item">
                    ...
                    <span class="library-count">${escapeHtml(lib.count)} items</span>
                </div>
            `;
        }
    }
            ...
            <tr id="job-row-${escapeHtml(job.id)}">
                ...
                <td>
                    ${job.status === 'running' || job.status === 'pending' ? 
                        ...
                        `<button class="btn btn-sm btn-outline-secondary" onclick="deleteJob('${escapeHtml(job.id)}')">
...
function showLogsModal() {
    ...
    document.getElementById('logsJobId').textContent = `Job ID: ${currentJobId}`;
...
async function refreshLogs() {
    ...
    try {
        const data = await apiGet(`/api/jobs/${currentJobId}/logs`);
...
async function startNewJob() {
    ...
    const forceCheckbox = document.getElementById('jobForceRegenerate');
    ...
    try {
        const result = await apiPost('/api/jobs', {
            ...
            config: {
                force_generate: forceCheckbox.checked
            }
        });
