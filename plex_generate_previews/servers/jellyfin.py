...
class JellyfinServer(MediaServer):
    ...
    def resolve_remote_path_to_item_id(self, remote_path: str) -> str | None:
        """Search Jellyfin for an item whose stored ``Path`` matches ``remote_path``.

        Same shape as :meth:`EmbyServer.resolve_remote_path_to_item_id` —
        Jellyfin shares Emby's ``/Items?searchTerm=...`` surface. Used by
        the secondary-publisher fan-out so the trickplay manifest (which
        is keyed by item id) can be written without a Jellyfin webhook
        having fired.
        """
        import os as _os
        ...
        try:
            ...
            logger.debug("Jellyfin reverse-lookup search failed for {}: {}", remote_path, exc)
            return None
